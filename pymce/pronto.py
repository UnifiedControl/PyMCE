...
def ConvertProntoRC5ToIrCode(prontoData, nRepeat=0):
    ...
    for j in range(nRepeat + 1):
        toggle = nRepeat % 2 == 0
        if prontoData[5] > 63:
            RC5String += EncodeBits(2, 1, 0, '10', '01')
        else:
            RC5String += EncodeBits(3, 1, 0, '10', '01')
        if toggle:
            RC5String += EncodeBits(1, 0, 0, '10', '01')
        else:
            RC5String += EncodeBits(0, 0, 0, '10', '01')
        RC5String += EncodeBits(prontoData[4], 4, 0, '10', '01')
        RC5String += EncodeBits(prontoData[5], 5, 0, '10', '01')
...
def ConvertProntoRC5XToIrCode(prontoData, nRepeat):
    if not (len(prontoData) == 7 or (len(prontoData) == 8 and prontoData[7] == 0x0000)) or prontoData[
                                                                                           0] != 0x5001: #CodeType RC5X
    ...
    for j in range(nRepeat + 1):
        toggle = nRepeat % 2 == 0
        if prontoData[5] > 63:
            RC5XString += EncodeBits(2, 1, 0, '10', '01')
        else:
            RC5XString += EncodeBits(3, 1, 0, '10', '01')
        if toggle:
            RC5XString += EncodeBits(1, 0, 0, '10', '01')
        else:
            RC5XString += EncodeBits(0, 0, 0, '10', '01')
        RC5XString += EncodeBits(prontoData[4], 4, 0, '10', '01')
        RC5XString += '0000'
        RC5XString += EncodeBits(prontoData[5], 5, 0, '10', '01')
        RC5XString += EncodeBits(prontoData[6], 5, 0, '10', '01')
...
def ConvertProntoRC6ToIrCode(prontoData, nRepeat):
    ...
    for j in range(nRepeat + 1):
        toggle = nRepeat % 2 == 0
        RC6String += '1111110010010101'
        if toggle:
            RC6String += '1100'
        else:
            RC6String += '0011'
        RC6String += EncodeBits(prontoData[4], 7, 0, '01', '10')
        RC6String += EncodeBits(prontoData[5], 7, 0, '01', '10')
...
def ConvertProntoRC6AToIrCode(prontoData, nRepeat):
    ...
    for j in range(nRepeat + 1):
        toggle = nRepeat % 2 == 0
        RC6AString += '11111110010101001'
        if toggle:
            RC6AString += '1100'
        else:
            RC6AString += '0011'
        if prontoData[4] > 127:
            RC6AString += EncodeBits(1, 0, 0, '01', '10')
            RC6AString += EncodeBits(prontoData[4], 14, 0, '01', '10')
        else:
            RC6AString += EncodeBits(0, 0, 0, '01', '10')
            RC6AString += EncodeBits(prontoData[4], 6, 0, '01', '10')
        RC6AString += EncodeBits(prontoData[5], 7, 0, '01', '10')
        RC6AString += EncodeBits(prontoData[6], 7, 0, '01', '10')
